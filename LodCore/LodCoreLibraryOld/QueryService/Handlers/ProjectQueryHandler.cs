        ...
        IQueryHandler<AllProjectsQuery, AllProjectsView>
    {
        ...
        public FullProjectView Handle(GetProjectQuery query)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                result = connection
                    .Query<ProjectDto, ImageDto, ProjectMembershipDto, ProjectLinkDto, ProjectTypeDto, ProjectDto>(
                        query.Sql,
                        (project, screenshot, projectMembership, projectLink, projectType) =>
                        {
                            ...
                            if (screenshot != null &&
                                !projectEntry.Screenshots.Any(s => s.BigPhotoUri == screenshot.BigPhotoUri))
                                projectEntry.Screenshots.Add(screenshot);

                            if (projectMembership != null &&
                                !projectEntry.Developers.Any(d => d.MembershipId == projectMembership.MembershipId))
