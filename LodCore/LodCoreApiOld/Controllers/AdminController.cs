...
namespace LodCoreApiOld.Controllers
{
    public class AdminController : ApiController
    {
        ...
        public IHttpActionResult ConfirmDeveloper(int userId)
        {
            ...
            {
                Log.Error("Failed to get user with id={0}. {1} StackTrace: {2}", userId.ToString(), ex.Message,
                    ex.StackTrace);
                return NotFound();
            }
            catch (InvalidOperationException ex)
            {
                Log.Error("Failed to confirm user with id={0}. {1} StackTrace: {2}", userId.ToString(), ex.Message,
                    ex.StackTrace);
                return Conflict();
            }
        ...
        public IHttpActionResult ChangeUserRegistrationDate(int id, string newRegistrationDate)
        {
            ...
            var account = _userManager.GetUser(id);
            account.RegistrationTime = new DateTime(Convert.ToInt32(dateComponents[2]),
                Convert.ToInt32(dateComponents[1]), Convert.ToInt32(dateComponents[0]));
            _userManager.UpdateUser(account);
            return Ok();
        }
